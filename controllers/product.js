...
exports.createProduct = (req, res) => {
    ...
    form.parse(req, (err, fields, file) => {
        ...
        if (file.photo) {
            ...
            product.image.data = fs.readFileSync(file.photo.path);
            product.image.contentType = file.photo.type;
        }
        // console.log(product);
...
exports.Updateprodcut = (req, res) => {
    ...
    form.parse(req, (err, fields, file) => {
        ...
        console.log(name, description, price, Availabelstock, category);
        if (err) {
            return res.status(404).json({ errormsg: "File not Found" })
        }
        ...
        tshirt.save((err, tshirt) => {
            if (!tshirt || err) {
                return res.status(404).json({ errormsg: "failed to Update tshirt in DB" })
            }
            res.status(200).json(tshirt)
        })
    })
...
exports.getAllproducts = (req, res) => {
    ...
    Product.find({}).select('-image').populate("category").sort([[sortBy, "asc"]]).limit(limit).then((product) => {
        return res.status(200).json(product)
    }).catch((err) => {
        return res.status(404).json({ errormsg: `Failed to load a products ${err}` })
    })
}
    ...
    let Bulkoperations = req.body.order.products.map((product) => {
        return {
            updateOne: {
                ...
                update: { $inc: { Availabelstock: -product.quantity, soldStocks: +product.quantity } }
            }
        }
    })
    Product.bulkWrite(Bulkoperations, {}, (err, products) => {
        if (err) {
            return res.status(404).json({ errormsg: `bulk operations failed ${err}` })
        }
        next()
    })
